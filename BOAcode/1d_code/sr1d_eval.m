...
function fr = sr1d_eval(params1D, figure_flag,noise_type)
if nargin < 2
    figure_flag = false;
end
...
switch noise_type
    ...
    case {'colored_gaussian'}
        % Unit-variance AR(1) process:
        % u_k = rho*u_{k-1} + sqrt(1-rho^2)*epsilon_k
        % epsilon_k ~ N(0,1), |rho| < 1.
        % rho controls the temporal correlation of noise.
        % Then rescale u_k to zero mean and standard deviation sigma.
        rho = 0.5;%0.2\0.5
        eps = randn(K,1);
        Noi = zeros(K,1);
        for k = 2:K
            Noi(k) = rho * Noi(k-1) + sqrt(1-rho^2) * eps(k);
        end
        % Normalize to the same standard sigma
        Noi = Noi - mean(Noi);
        Noi = sigma * Noi / std(Noi);
...
if figure_flag==2
    ...
    xlabel('t/s'); ylabel('x');
    xlim([0 420]);
    title(['Waveform of SR-opt output']);
    grid on;
end
