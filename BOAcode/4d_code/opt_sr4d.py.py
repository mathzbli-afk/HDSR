...
def generate_mcmc_samples(num_samples, dim, bounds):
    def log_prob(params):
        ...
        try:
            input_array = [float(x) for x in params]
            snri = eng.sr4d_eval(matlab.double(input_array), False, noise_type,nargout=1)
            snri = float(snri)
        except Exception:
            return -np.inf
    ...
    if flat_samples.shape[0] < num_samples:
        raise ValueError("Not enough MCMC samples after burn-in.")

    idx = np.random.choice(flat_samples.shape[0], size=num_samples, replace=False)
...
def objective_function(X):
    ...
    result = eng.sr4d_eval(matlab.double(input_array),False, noise_type,nargout=1)
...
# Noise setting
# Options should match the cases in sr4d_eval.m:
# 'white_gaussian', 'colored_gaussian'
noise_type = 'colored_gaussian'
# If colored Gaussian noise is used, manually set tauc in sr1d_eval.m.
# This label is only used for log file naming.
tauc_label = 'tauc_0.05'#can be changed to 'tauc_0.2'\'tauc_0.1'\’tauc_0.05‘
...
optimizer = BayesianOptimization(
    ...
    acquisition_weight = 1,  # Conservative # LCB balance parameter (exploration/exploitation trade-off)
...
# Log
log_path = f"boa_log_sr4d_{noise_type}_{tauc_label}.txt"
with open(log_path, "w", encoding="utf-8") as f:
    f.write("Bayesian Optimization Log for SR-4D\n")
    f.write(f"Noise type: {noise_type}\n")
    f.write(f"tauc label: {tauc_label}\n")
